...
void sendAsyncSolution(tspSolution_t* solution, int to) {
    ...
    MPI_Isend(&solution->hasSolution, 1, MPI_C_BOOL, to, SOLUTION_TAG, MPI_COMM_WORLD, &requests[1]);
...
void recvAsyncSolution(tspSolution_t* solution, int from) {
    ...
    MPI_Irecv(&solution->hasSolution, 1, MPI_C_BOOL, from, SOLUTION_TAG, MPI_COMM_WORLD, &requests[1]);
